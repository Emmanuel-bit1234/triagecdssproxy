...
import { users, conversations, conversationParticipants, messages, messageReads } from '../db/schema.js';
...
async function getUnreadCount(conversationId, userId) {
    ...
    const unreadResult = await db
        .select({ count: count() })
        .from(messages)
        .where(and(eq(messages.conversationId, conversationId), gte(messages.createdAt, lastReadAt), isNull(messages.deletedAt)));
...
async function getOrCreateDirectConversation(userId1, userId2) {
    ...
    const user1Conversations = await db
        ...
        .where(and(eq(conversations.type, 'direct'), eq(conversationParticipants.userId, userId1)));
    if (user1Conversations.length > 0) {
        ...
        const matchingConv = await db
            ...
            .from(conversationParticipants)
            .where(and(sql `${conversationParticipants.conversationId} = ANY(${conversationIds})`, eq(conversationParticipants.userId, userId2)))
            .limit(1);
...
messagesRoute.get('/conversations', authMiddleware, async (c) => {
    try {
        ...
        const conditions = [eq(conversationParticipants.userId, user.id)];
        if (type) {
        ...
        const userConvs = await db
            ...
            .limit(limit)
            .offset(offset);
        const conversationList = await Promise.all(userConvs.map(async (convData) => {
            ...
            if (conv.type === 'direct') {
                ...
                const otherParticipant = await db
                    ...
                    .from(conversationParticipants)
                    .innerJoin(users, eq(conversationParticipants.userId, users.id))
                    .where(and(eq(conversationParticipants.conversationId, convId), sql `${conversationParticipants.userId} != ${user.id}`))
                    .limit(1);
                return {
                    id: conv.id,
                    type: conv.type,
...
messagesRoute.get('/conversations/:conversationId/messages', authMiddleware, async (c) => {
    try {
        ...
        const messagesWithSenders = await Promise.all(messagesList.map(async (msg) => {
...
messagesRoute.post('/groups', authMiddleware, adminMiddleware, async (c) => {
    try {
        ...
        const conversationId = newConversation[0].id;
        // Add participants
        await db.insert(conversationParticipants).values(body.userIds.map(userId => ({
            conversationId,
            userId,
        })));
        // Get participants with user info
...
messagesRoute.get('/groups/:groupId', authMiddleware, async (c) => {
    try {
        ...
        const group = await db
            .select()
            .from(conversations)
            .where(and(eq(conversations.id, groupId), eq(conversations.type, 'group')))
            .limit(1);
        if (group.length === 0) {
...
messagesRoute.post('/groups/:groupId/participants', authMiddleware, adminMiddleware, async (c) => {
    try {
        ...
        const existingParticipants = await db
            ...
            .from(conversationParticipants)
            .where(and(eq(conversationParticipants.conversationId, groupId), inArray(conversationParticipants.userId, body.userIds)));
        ...
        await db.insert(conversationParticipants).values(newUserIds.map(userId => ({
...
messagesRoute.delete('/groups/:groupId/participants/:userId', authMiddleware, adminMiddleware, async (c) => {
    try {
        ...
        const participant = await db
            .select()
            .from(conversationParticipants)
            .where(and(eq(conversationParticipants.conversationId, groupId), eq(conversationParticipants.userId, userId)))
            .limit(1);
        if (participant.length === 0) {
...
messagesRoute.get('/groups', authMiddleware, adminMiddleware, async (c) => {
    try {
        ...
        const groupsWithDetails = await Promise.all(groupsList.map(async (group) => {
            const participantCount = await db
                .select({ count: count() })
                .from(conversationParticipants)
