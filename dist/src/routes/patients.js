...
const getNextPatientNumber = async () => {
    const result = await db
        .select({ maxNumber: sql `MAX(CAST(SUBSTRING(patient_number, 2) AS INTEGER))` })
        .from(patients);
...
const checkPatientExists = async (firstName, lastName, dateOfBirth) => {
    const existingPatient = await db
        .select()
        .from(patients)
        .where(and(eq(patients.firstName, firstName), eq(patients.lastName, lastName), eq(patients.dateOfBirth, dateOfBirth)))
        .limit(1);
...
patientsRoute.post('/', authMiddleware, async (c) => {
    try {
        ...
        const existingPatient = await checkPatientExists(patientData.firstName, patientData.lastName, patientData.dateOfBirth);
        if (existingPatient) {
            return c.json({
                success: false,
                exists: true,
    ...
    try {
        ...
        const patientWithPredictions = {
            ...
            predictions: predictions.map(p => ({
                ...
                inputs: p.inputs,
                predict: p.predict,
                ktasExplained: {}, // Will be populated from actual data
                probs: p.probs,
...
patientsRoute.get('/search', authMiddleware, async (c) => {
    try {
        ...
        const searchResults = await db
            .select()
            .from(patients)
            .where(and(eq(patients.isActive, true), or(ilike(patients.firstName, `%${query}%`), ilike(patients.lastName, `%${query}%`), ilike(patients.patientNumber, `%${query}%`), ilike(sql `CONCAT(${patients.firstName}, ' ', ${patients.lastName})`, `%${query}%`))))
