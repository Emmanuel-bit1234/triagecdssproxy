...
const nurseReportsRoute = new Hono<{ Variables: AuthVariables }>();

// Interface for nurse report response
...
nurseReportsRoute.get('/:nurseId', authMiddleware, async (c) => {
  try {
    ...
    // Build date filters
    let dateFilter: any = eq(predictionLogs.userId, nurseId);
    let periodType: 'daily' | 'overall' = 'overall';
    let periodInfo: any = {};
    ...
    if (date) {
      ...
      dateFilter = and(
        eq(predictionLogs.userId, nurseId),
        gte(predictionLogs.createdAt, startDate),
        lte(predictionLogs.createdAt, endDate)
      );
      periodType = 'daily';
      periodInfo = {
        type: 'daily',
    ...
    const arrivalModeStats = await db
      .select({
        mode: sql<number>`CAST((${predictionLogs.inputs}->>'Arrival_mode') AS INTEGER)`,
        count: count(predictionLogs.id),
      })
      .from(predictionLogs)
      .where(dateFilter)
    ...
    const chiefComplaintStats = await db
      .select({
        complaint: sql<string>`${predictionLogs.inputs}->>'Chief_complain'`,
        count: count(predictionLogs.id),
      })
      .from(predictionLogs)
      .where(dateFilter)
    ...
    const painStats = await db
      .select({
        painLevel: sql<number>`CAST((${predictionLogs.inputs}->>'Pain') AS INTEGER)`,
        count: count(predictionLogs.id),
      })
      .from(predictionLogs)
      .where(dateFilter)
    ...
    const vitalSignsStats = await db
      .select({
        sbp: sql<number>`CAST((${predictionLogs.inputs}->>'SBP') AS INTEGER)`,
        dbp: sql<number>`CAST((${predictionLogs.inputs}->>'DBP') AS INTEGER)`,
        hr: sql<number>`CAST((${predictionLogs.inputs}->>'HR') AS INTEGER)`,
        rr: sql<number>`CAST((${predictionLogs.inputs}->>'RR') AS INTEGER)`,
        bt: sql<number>`CAST((${predictionLogs.inputs}->>'BT') AS REAL)`,
      })
      .from(predictionLogs)
      .where(dateFilter);
    ...
    const hourlyStats = await db
      .select({
        hour: sql<number>`EXTRACT(HOUR FROM ${predictionLogs.createdAt})`,
        count: count(predictionLogs.id),
      })
      .from(predictionLogs)
      .where(dateFilter)
    ...
    const mostCommonLevel = Object.entries(levelDist).reduce((a, b) => 
      levelDist[Number(a[0])] > levelDist[Number(b[0])] ? a : b
    )[0];
    ...
    const arrivalModeDist: Record<number, number> = {};
    ...
    const painDist: Record<number, number> = {};
    painStats.forEach(stat => {
      painDist[stat.painLevel] = Number(stat.count);
    });

    // Vital signs analysis
    const validVitals = vitalSignsStats.filter(v => 
      v.sbp && v.dbp && v.hr && v.rr && v.bt
    );
    ...
    const highPain = Object.entries(painDist).reduce((sum, [level, count]) => 
      sum + (Number(level) >= 7 ? Number(count) : 0), 0
    );
    ...
    const busiestHour = Object.entries(hourlyDist).reduce((a, b) => 
      hourlyDist[Number(a[0])] > hourlyDist[Number(b[0])] ? a : b
    )[0];
    const leastBusyHour = Object.entries(hourlyDist).reduce((a, b) => 
      hourlyDist[Number(a[0])] < hourlyDist[Number(b[0])] ? a : b
    )[0];
    ...
    confidenceStats.forEach(stat => {
      const probs = stat.probs as string[];
