...
async function generateNextPatientNumber(): Promise<string> {
  try {
...
patientsRoute.post('/', authMiddleware, async (c) => {
  try {
    ...
    const body = await c.req.json() as CreatePatientRequest;
    ...
    let processedNotes: PatientNote[] | null = null;
...
patientsRoute.put('/:id', authMiddleware, async (c) => {
  try {
    const user = c.get('user');
    const id = parseInt(c.req.param('id'));
    const body = await c.req.json() as UpdatePatientRequest;
    ...
    // Process notes
    let processedNotes: PatientNote[] | null | undefined = undefined;
    ...
    if (processedNotes !== undefined) updateData.notes = processedNotes;
...
patientsRoute.get('/', authMiddleware, async (c) => {
  try {
    ...
    const gender = c.req.query('gender') as 'male' | 'female' | 'other' | undefined;
...
patientsRoute.get('/stats/overview', authMiddleware, async (c) => {
  try {
    ...
    genderStats.forEach(stat => {
      if (stat.gender === 'male') genderDistribution.male = Number(stat.count);
      else if (stat.gender === 'female') genderDistribution.female = Number(stat.count);
      else if (stat.gender === 'other') genderDistribution.other = Number(stat.count);
    });
    ...
    allergyStats.forEach(stat => {
      if (stat.allergies && Array.isArray(stat.allergies)) {
        stat.allergies.forEach((allergy: string) => {
    ...
    medicationStats.forEach(stat => {
      if (stat.medications && Array.isArray(stat.medications)) {
        stat.medications.forEach((medication: string) => {
